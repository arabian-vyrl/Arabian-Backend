...
const blogSchema = new mongoose.Schema(
  {
    ...
    // Images embedded in the content (from React Quill)
    bodyImages: [
      {
        url: String,
        filename: String,
...
blogSchema.statics.findPublished = function () {
  return this.find({ status: "published", isPublished: true }).sort({
    publishedAt: -1,
  });
};

blogSchema.statics.findByAgentEmail = function (agentEmail) {
  return this.find({ "author.agentEmail": agentEmail }).sort({ createdAt: -1 });
};

blogSchema.statics.findByTags = function (tags) {
