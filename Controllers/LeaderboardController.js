...
//   // Function to check record type and get appropriate date
//   // to map monthly commission for agent
...
// controller/cronJobsController.js (or wherever this lives)

async function syncMonthlyPropertiesJobNew() {
  try {
    console.log("🔄 [CRON] Starting monthly properties update...");

    // 🔹 Helper to detect relisted properties (same logic as the static)
    const isRelistedProperty = (propertyId) => {
    ...
    for (const agent of agents) {
      ...
      // Split into original vs relisted using the same rules
      const originalProps = liveProperties.filter((p) => {
        const id = p.propertyId || "";
        return !isRelistedProperty(id);
      });
