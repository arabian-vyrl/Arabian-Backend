...
const MatchGeoPointOptimized = async (forceRematch = false) => {
  ...
  try {
    ...
    const [
      ...
      ExtractRedinLocation.find({}).lean(),
    ...
    const summary = {
      ...
      alreadyMatchedSkipped: alreadyMatchedCount,
      propertiesToProcess: dubaiLiveProperties.length,
      totalRedinLocations: redinLocationData.length,
      totalRedinPropertiesFlattened: redinPropertiesFlattened.length,
      steps: {}, // per step info goes here
      totalMatched: 0,
      totalUnmatched: 0,
      dbUpdateSuccess: 0,
      dbUpdateFailed: 0,
    ...
    const matchingSteps = [
      { num: 1, name: "combined_region_match", fn: executeStep1_CombinedRegionMatch },
      ...
      { num: 4, name: "second_region_match", fn: executeStep4_SecondRegionMatch },
      { num: 5, name: "partial_word_match", fn: executeStep5_PartialWordMatch },
      { num: 6, name: "starting_2_words_match", fn: executeStep6_Starting2WordsMatch },
      { num: 7, name: "coordinate_match", fn: executeStep7_CoordinateMatch },
    ];

    // ✅ FIX: Pass parameters in correct order with stats object
    for (const step of matchingSteps) {
      unmatchedProperties = await executeMatchingStepNonBlocking(
        ...
        stats,                      // ✅ correct
        results,                    // ✅ correct
        bulkOperations,             // ✅ correct
        subtypeMismatchTrackedIds,  // ✅ correct
        successfullyMatchedIds,     // ✅ correct
      );
    }

    // ✅ totals (simple)
    summary.totalMatched =
      (results.step1_matched.length || 0) +
      (results.step2_matched.length || 0) +
      (results.step3_matched.length || 0) +
      (results.step4_matched.length || 0) +
      (results.step5_matched.length || 0) +
      (results.step6_matched.length || 0) +
      (results.step7_matched.length || 0);
    ...
    if (bulkOperations.length > 0) {
      ...
      for (let i = 0; i < bulkOperations.length; i += BULK_CHUNK_SIZE) {
        ...
        try {
          ...
          summary.dbUpdateSuccess += result.modifiedCount || chunk.length;
          console.log(
          ...
          summary.dbUpdateFailed += chunk.length;
        }
    ...
    summary.totalUnmatched = results.unmatched.length;
    ...
    return {
      ...
      summary: {
        ...summary,
        stats, // Include stats in summary
        steps: Object.values(summary.steps), // nicer response as array
      },

      // still giving these (optional but consistent)
      stepMatchedCounts: {
        step1: results.step1_matched.length,
        step2: results.step2_matched.length,
        step3: results.step3_matched.length,
        step4: results.step4_matched.length,
        step5: results.step5_matched.length,
        step6: results.step6_matched.length,
        step7: results.step7_matched.length,
      },

      unmatched: {
        total: summary.totalUnmatched,
        nameMatchedButSubtypeNotMatch,
        nameNotMatchedAtAll,
      },
    };
